...
def optimize():
    ...
    if "response_type" in input_json:
        ...
        if type(response_type) not in [str] or (response_type != "shares" and response_type != "weights"):
            flask.abort(400, description="Error: Response type must be either 'shares' or 'weights'.")
    ...
    if "universe" in input_json:
        ...
        if type(universe) not in [list]:
            flask.abort(400, description="Error: Universe must be a list of tickers.")
        for ticker in unclean_universe:
            if type(ticker) not in [str] or ticker.upper() not in TICKER_UNIVERSE:
                flask.abort(400, description=f'Error: Invalid ticker "{str(ticker)}".')
    ...
    if "initial_holdings" in input_json:
        ...
        if type(unclean_initial_holdings) not in [dict]:
            flask.abort(400, description="Error: Initial holdings must be a dictionary.")
        for ticker, shares in unclean_initial_holdings.items():
            if type(ticker) not in [str] or ticker.upper() not in TICKER_UNIVERSE:
                flask.abort(400, description=f'Error: Invalid ticker "{str(ticker)}" in initial holdings.')
            if type(shares) not in [int, float] or shares < 0.000:
                flask.abort(400, description=f'Error: Invalid number of shares "{str(shares)}" in initial holdings.')
        initial_holdings = {s.upper(): unclean_initial_holdings[s] for s in unclean_initial_holdings}
